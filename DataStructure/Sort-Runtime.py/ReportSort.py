...
# def time_aN(arr):
#     def recursive_fibonacci(n=len(arr)):
#         if n <= 1:
#             return n
#         else:
...
while 10001 > k:
    ...
    # start = time.perf_counter()
    # time_NNN(array)
    # timeNNN.append(time.perf_counter() - start)

    # # # O(a^N)
    # start = time.perf_counter()
    # time_aN(array)
    # timeaN.append(time.perf_counter() - start)
...
# plt.plot(x, timeNNN, label='O(n^3)')
# plt.plot(x, timeaN, label='O(a^N)')
